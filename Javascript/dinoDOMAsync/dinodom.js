...
// Meteorito acércandose a cada segundo (se modifica la propiedad top y right /sec ), <img> position:absolute.

function crearMeteorito() {

    let contenendorMeteorito = document.querySelector("#meteorito");
    let imagenMeteorito = document.createElement("IMG");
    imagenMeteorito.src = "./img/meteor.jpg";
    contenendorMeteorito.appendChild(imagenMeteorito);

}

function cambiarPosicionMeteorito(positionTop,positionRight){
    let meteorito = document.querySelector("#meteorito");
    meteorito.style.position = "abolute";
    meteorito.style.top = positionTop+"px";
    meteorito.style.right = positionRight+"px";
